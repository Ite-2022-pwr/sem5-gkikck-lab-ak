...
def render(time):
    ...
    glTexImage2D(
        GL_TEXTURE_2D, 0, 3, textures[N_key_pressed].size[0], textures[N_key_pressed].size[1], 0,
        GL_RGB, GL_UNSIGNED_BYTE, textures[N_key_pressed].tobytes("raw", "RGB", 0, -1)
    )
...
def keyboard_key_callback(window, key, scancode, action, mods):
    ...
    if key == GLFW_KEY_N and action == GLFW_PRESS:
        N_key_pressed = (N_key_pressed + 1) % 2
