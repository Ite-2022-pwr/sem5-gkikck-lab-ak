...
def compile_shaders():
    vertex_shader_source = """
        ...
        float rand() {
            ...
            return fract(sin(dot(vec2(gl_InstanceID, gl_VertexID), vec2(12.9898,78.233))) * 43758.5453123);
        }

        vec4 transform() {
            float x = gl_InstanceID / 10;
            float y = gl_InstanceID % 10;

            return position + vec4(x + rand(), y + rand(), rand(), 0);
        }

        void main(void) {
            gl_Position = P_matrix * V_matrix * M_matrix * transform();
            vertex_color = color;
        }
    """
